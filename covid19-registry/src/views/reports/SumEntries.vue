<template>
  <div>
    <div class="row">
      <div class="col-12">
        <h1 class="text-left">
          <i class="fas fa-file-invoice mr-2"></i>Entry Sum by Days
        </h1>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async fetchEntries() {
      try {
        ...
        const res = await axios.get(
          `/entry/fetchSumEntries?dateStart=${this.dateStart}&dateEnd=${this.dateEnd}`
        );
    ...
    async exportExcel() {
      const url = `/entry/exportExcelSumEntries?dateStart=${this.dateStart}&dateEnd=${this.dateEnd}`;
